...
type FlightLogDatabase interface {
	...
	CreateConnection(username string, password string, port string, hostname string)
	...
	GetAllUserGroups(limit int) ([]UserGroup, error)
	...
	GetAllFlights(limit int) ([]Flight, error)
	...
	GetFlightIncidents(limit int) ([]Flight, error)
	...
	GetAllWings(limit int) (Wing, error)
	...
	GetAllStartSites(limit int) ([]StartSite, error)
}
